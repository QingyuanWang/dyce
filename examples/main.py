...
import utils
from utils import get_args_parser, setup_for_distributed
...
def main(args, accelerator):
    ...
    # model_base = torch.compile(model_base, dynamic=False, fullgraph=False)
    model = dmc.MultiExitNetowrk(
        model_base,
        num_classes=args.nb_classes,
    ...
    if args.eval:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if args.eval_interval != 0 and (epoch + 1) % args.eval_interval == 0 and data_loader_val is not None:
                test_stats = evaluate(data_loader_val, model, accelerator, use_amp=args.use_amp)
