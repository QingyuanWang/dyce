...
def compute_loss(output, ee_out, targets):
...
def train_one_epoch(
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        targets = targets.to(accelerator.device).unsqueeze(-1)
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            ee_loss, ori_loss, ee_loss_raw = compute_loss(output, ee_out, targets)
...
@torch.no_grad()
def evaluate(data_loader, model, accelerator, use_amp=False):
